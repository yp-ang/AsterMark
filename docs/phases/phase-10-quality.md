...
- [x] 10.2 Golden images (`Tests/AsterCoreTests/Golden`): 5 blend modes, shadow, rotation, tile, and crop + resize + sharpen, each compared with its stored reference (±3/255 per channel, at most 0.5% of pixels beyond that, to allow for GPU rounding between Macs). Regenerate with `UPDATE_GOLDEN=1 make test`. Orientation and presets are covered by exact pixel tests elsewhere.
