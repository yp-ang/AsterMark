...
| D20 | 2026-09-24 | Text layers reuse `Layer` with `text` set and a zero `watermarkID`, rather than a new content enum. | Keeps every existing call site and file format working. The custom decoder defaults all newer fields. | Accepted |
| D21 | 2026-09-24 | The canvas shows tiled layers as a Core Image render of the export pipeline. | Exact preview/export parity for a pattern that's hard to reproduce with replicator layers. Tiles aren't dragged, so a static image is enough. | Accepted |
